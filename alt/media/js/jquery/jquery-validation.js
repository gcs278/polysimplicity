				...
				function handle() {
					if ( validator.settings.submitHandler ) {
						if (validator.submitButton) {
							...
							var hidden = $("<input type='hidden'/>").attr("id", validator.submitButton.id).val(validator.submitButton.value).appendTo(validator.currentForm);
						}
...
$.extend($.validator, {
	...
	messages: {
        ...
        dateMoreThan: $.validator.format("Please enter a Date more than or equal to {0}.")
	},
	...
	prototype: {
		...
		init: function() {
			...
			$.each(this.settings.groups, function(key, value) {
				$.each(value.split(/\s/), function(index, id) {
					groups[id] = key;
				});
			});
	...
	classRuleSettings: {
		...
		validarUserName:{validarUserName: true},
		creditcard: {creditcard: true},
		cedulaEcuador:{cedulaEcuador: true}
	},
	...
	addMethod: function(id, method, message) {
		$.validator.methods[id] = method;
		$.validator.messages[id] = message != undefined ? message : $.validator.messages[id];
		if (method.length < 3) {
			$.validator.addClassRules(id, $.validator.normalizeRule(id));
		}
	},
	...
	methods: {
		...
		percentage: function(value, element) {
			return this.optional(element) || /(^(100(?:\.0{1,2})?))|(?!^0*$)(?!^0*\.0*$)^\d{1,2}(\.\d{1,2})?$/.test(value);
		},
		...
		alphaNumeric: function(value, element){
			return this.optional(element) || /^[a-zA-Z0-9_\u00C1\u00E1\u00C9\u00E9\u00CD\u00ED\u00D3\u00F3\u00DA\u00FA\u00DC\u00FC\u00D1\u00F1 ]+$/.test(value);
		},
		alphaNumericSpecial: function(value, element){
			return this.optional(element) || /(^[a-zA-Z0-9_\u00C1\u00E1\u00C9\u00E9\u00CD\u00ED\u00D3\u00F3\u00DA\u00FA\u00DC\u00FC\u00D1\u00F1 ]+(?:[- # a-zA-Z0-9_\.\u00C1\u00E1\u00C9\u00E9\u00CD\u00ED\u00D3\u00F3\u00DA\u00FA\u00DC\u00FC\u00D1\u00F1 ])*)$/.test(value);
		},
		...
		dateLessThan: function(value, element, param) {
			...
			param=parseDate(param);
			return value < param;
		},

        dateMoreThan: function(value, element, param) {
			value=parseDate(value);
			param=parseDate(param);
			return value > param;
		},
